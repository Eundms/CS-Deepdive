...
## GC 튜닝 가이드
throughput goal을 잡고 max heap size를 잡는다 (swap 없이)
그럼에도 목표를 달성하지 못하면 RAM 이 이 throughput 을 감당하지 못하는 것이다 
달성했는데 pause time이 너무 길면 maximum pause-time goal 을 지정한다 
(단, throughput goal과 maximum pause-time goal은 트레이드 오프 관계이기 때문에 적당한 타협이 필요하다)
서버 애플리케이션은 동일하게 설정한다 
